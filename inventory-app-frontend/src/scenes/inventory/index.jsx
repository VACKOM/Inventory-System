...
const Inventory = () => {
        ...
        useEffect(() => {
            if (products.length > 0 && stock.length > 0) {
                const combinedData = products.map(product => {
                    ...
                    return {
                        id: product.sku,  // Add an 'id' field for the DataGrid
                        sku: product.sku,
                        name: product.name,
                        description: product.description,
                        price: stockItem ? stockItem.price : 0,
                        quantity: stockItem ? stockItem.quantity : 0,
                        category: product.category,  // Make sure this matches the column field name
                    };
                });
  ...
  return (
    <Box m="20px">
      <Header title="Inventory List" subtitle="Managing the Inventory" />
      <Box
        m="40px 0 0 0"
        height="75vh"
        sx={{
      ...
      >
        <DataGrid checkboxSelection rows={inventoryList} columns={columns} />
      </Box>
    </Box>
  );
};
